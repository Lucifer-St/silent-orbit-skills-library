...
Use only the [`v0.11.0-beta.9` GitHub Pre-release](https://github.com/Lucifer-St/silent-orbit-skills-library/releases/tag/v0.11.0-beta.9).
...
docker run --rm -e HOME=/tmp/home -e SILENT_ORBIT_SCENARIO=unmounted \
  --mount type=bind,src="$PWD/silent-orbit-skills-library-0.11.0-beta.9.tgz",dst=/input/release.tgz,readonly \
...
docker run --rm -e HOME=/tmp/home -e SILENT_ORBIT_SCENARIO=unmounted `
  --mount "type=bind,src=$assets\silent-orbit-skills-library-0.11.0-beta.9.tgz,dst=/input/release.tgz,readonly" `
