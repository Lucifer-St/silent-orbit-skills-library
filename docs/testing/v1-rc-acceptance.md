...
Use only the [`v0.12.0-beta.1` GitHub Pre-release](https://github.com/Lucifer-St/silent-orbit-skills-library/releases/tag/v0.12.0-beta.1).
...
Expected CLI version: `0.5.0`. The package version and CLI interface version
...
docker run --rm -e HOME=/tmp/home -e SILENT_ORBIT_SCENARIO=unmounted \
  --mount type=bind,src="$PWD/silent-orbit-skills-library-0.12.0-beta.1.tgz",dst=/input/release.tgz,readonly \
...
docker run --rm -e HOME=/tmp/home -e SILENT_ORBIT_SCENARIO=unmounted `
  --mount "type=bind,src=$assets\silent-orbit-skills-library-0.12.0-beta.1.tgz,dst=/input/release.tgz,readonly" `
