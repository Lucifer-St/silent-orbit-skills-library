...
Use only the [`v0.11.0-beta.6` GitHub Pre-release](https://github.com/Lucifer-St/silent-orbit-skills-library/releases/tag/v0.11.0-beta.6).
...
$actual = (Get-FileHash .\silent-orbit-skills-library-0.11.0-beta.6.tgz -Algorithm SHA256).Hash.ToLowerInvariant()
...
```sh
npm init -y
npm install ./silent-orbit-skills-library-0.11.0-beta.6.tgz
npx silent-orbit --version
```

Expected CLI version: `0.4.0`. The package version and CLI interface version
...
docker run --rm -e HOME=/tmp/home -e SILENT_ORBIT_SCENARIO=unmounted \
  --mount type=bind,src="$PWD/silent-orbit-skills-library-0.11.0-beta.6.tgz",dst=/input/release.tgz,readonly \
...
docker run --rm -e HOME=/tmp/home -e SILENT_ORBIT_SCENARIO=unmounted `
  --mount "type=bind,src=$assets\silent-orbit-skills-library-0.11.0-beta.6.tgz,dst=/input/release.tgz,readonly" `
