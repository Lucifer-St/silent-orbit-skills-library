...
Public Generator 只通过已验证的 GitHub Pre-release tarball 分发，不发布到 npm registry。请按 [Generator 快速开始](./GENERATOR_QUICKSTART.zh-CN.md) 校验产物，从已下载文件进行项目级或全局 CLI 安装；如有需要，再以项目级方式安装随包提供的 `build-skill-cosmos` Agent Skill，并完成一次经过公开边界复核的首次生成。
